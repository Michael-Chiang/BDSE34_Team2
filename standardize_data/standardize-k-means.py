...
# 读取分群对照表
tech_clusterID_df = pd.read_csv('stock_all_info_technology_k-means.csv')
finance_clusterID_df = pd.read_csv('stock_all_info_finance_k-means.csv')
...
for clusterID, data in tech_clusterID.items():
    if clusterID in clusterID_to_plot and not data.empty:
        ...
        plt.plot(data.index, data['Average'], label=f'Technology k-means clusterID {clusterID}')
        plt.title(f'Technology k-means clusterID {clusterID} Average Adj Close')
        plt.xlabel('Date')
        plt.ylabel('Standardized Adj Close')
        plt.legend()
        plt.savefig(f'pic/tech/Technology k-means clusterID {clusterID}.png', format='png')
        plt.close()
...
for clusterID, data in finance_clusterID.items():
    if clusterID in clusterID_to_plot and not data.empty:
        data['Average'] = data.mean(axis=1)
        plt.figure(figsize=(14, 7))
        plt.plot(data.index, data['Average'], label=f'Finance k-means clusterID {clusterID}')
        plt.title(f'Financek-means clusterID {clusterID} Average Adj Close')
        plt.xlabel('Date')
        plt.ylabel('Standardized Adj Close')
        plt.legend()
        plt.savefig(f'pic/fin/Technology k-means clusterID {clusterID}.png', format='png')
        plt.close()
...
plt.title("Technology k-means' 5 cluster & QQQ Average Adj Close")
plt.xlabel('Date')
plt.ylabel('Standardized Adj Close')
plt.legend()
plt.savefig("pic/tech/Technology k-means' 5 cluster & QQQ.png", format='png')
plt.close()
...
plt.title("Finance k-means' 5 cluster & XLF Average Adj Close")
plt.xlabel('Date')
plt.ylabel('Standardized Adj Close')
plt.legend()
plt.savefig("pic/fin/Finance k-means' 5 cluster & XLF.png", format='png')
plt.close()